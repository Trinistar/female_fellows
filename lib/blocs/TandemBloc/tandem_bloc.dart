...
class TandemBloc extends Bloc<TandemEvent, TandemState> {
  TandemBloc(this._firestoreUserprofileRepository, this._authBloc)
      : super(TandemInitial()) {
    ...
    on<UnloadAllTandemsEvent>(
        (UnloadAllTandemsEvent event, Emitter<TandemState> emit) =>
            emit(TandemsNotLoaded()));
  ...
  Future<void> _onUpdateTandemFilterEvent(
      UpdateTandemFilterEvent event, Emitter<TandemState> emit) async {
    return emit.onEach(
      _firestoreUserprofileRepository
          .getAllTandems(event.profile.tandemTypeFilter.toString()),
      onData: (List<FFUser> tandems) async {
        if (tandems.isEmpty) return emit(TandemsNotLoaded());

        emit(TandemNewcomersLoaded(
            tandems: await _filterGeoTandems(event.profile, tandems)));
      },
    );
  }

  Future<void> _onLoadAllTandemLocalsEvent(
      LoadAllTandemLocalsEvent event, Emitter<TandemState> emit) async {
    return emit.onEach(
      _firestoreUserprofileRepository.getAllTandems('local'),
      onData: (List<FFUser> tandems) async {
        if (tandems.isEmpty) return emit(TandemsNotLoaded());

        final List<FFUser> current =
            await _filterGeoTandems(event.profile, tandems);
  ...
  Future<void> _onLoadAllTandemNewcomersEvent(
      LoadAllTandemNewcomersEvent event, Emitter<TandemState> emit) async {
    return emit.onEach(
  ...
  Future<List<FFUser>> _filterGeoTandems(
      FFUser profile, List<FFUser> tandems) async {
    ...
    if (profile.tandemTypeFilter == TandemTypeFilter.all ||
      ...
      if (profile.location != null && profile.location!.data != null) {
        ...
        for (final FFUser user in tandems) {
          ...
          user.location =
              await _firestoreUserprofileRepository.getUserLocation(user.id!);
  ...
  double _getMatchByAge(FFUser profile, FFUser tandem) {
    ...
    if (tandem.birthday != null && profile.birthday != null) {
      ...
      final Duration difference =
          ownBirthday.difference(tandem.birthday!.toDate());
